...
const nextGenFilter = require('../util/nextgenfilter');

const profile = function profile(cb) {
  let base = this.config.get('profiles:base');
  let profiles = this.program.profile;

  //Inject NextGen Profile
  if (process.argv.slice(2).indexOf('-I') >= 0) {
    let nextGenProfiles = nextGenFilter.getNextGenTestCases(this.config.get('profiles:base'), this.config.get('data'))
    profiles = Object.keys(nextGenProfiles) || profiles
    let parent = this;
    Object.keys(nextGenProfiles).forEach(function (profileName) {
      parent.config.set(`profiles:${profileName}`, nextGenProfiles[profileName]);
    })
  }
  //End NextGen Profile Injection
